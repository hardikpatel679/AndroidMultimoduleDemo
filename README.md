...
- **Domain**: Located in `:domain`. Contains pure Kotlin **UseCases**, **Repository Interfaces**, and **Functional Error Handling** models.
- **Data**: Located in `:data`. Implements repository interfaces, handles API calls via Retrofit, and manages data mapping.
- **Core UI**: Located in `:core:ui`. Centralizes the design system, UI components, and **Localized Error Mapping**.
...
- **DI**: Hilt (Dagger)
- **Automation**: Jenkins (Groovy DSL), GitHub Actions (v7), Firebase CLI
- **Network**: Retrofit 3 + OkHttp 5
- **Concurrency**: Kotlin Coroutines & Flow (Main-Safe architecture)
...
- **Functional API Result**: A custom `ApiResult` sealed class (`Loading`, `Success`, `Error`) used across layers for consistent state handling.
- **Type-Safe Error Handling**: Centralized `AppError` domain model mapped to localized `UiText` resources in the UI layer, eliminating hardcoded strings in logic.
- **Main-Safe UseCases**: UseCases explicitly handle threading via `.flowOn(Dispatchers.IO)`, ensuring the ViewModel remains focused on UI state.
